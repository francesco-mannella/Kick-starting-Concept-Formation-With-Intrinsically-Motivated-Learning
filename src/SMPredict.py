...
class SMPredict:
    ...
    def __init__(self, inp_num, out_num, lr=0.1, name="predict"): 
        ...
        self.out_layer = keras.layers.Dense(self.out_num, 
                name=f"{self.name}_layer",activation="sigmoid", 
    ...
    def weights(self):
        ...
        return self.out_layer.get_weights()

    def set_weights(self, weights):
        #keras.backend.set_value(self.model.get_layer(f"{self.name}_layer",).kernel, weights)
        self.model.get_layer(f"{self.name}_layer",).set_weights(weights)

    def spread(self, inp):
        assert len(inp.shape) == 2
        inp = tf.cast(inp, dtype="float32")
        comp = self.model.predict(inp, verbose=0)
