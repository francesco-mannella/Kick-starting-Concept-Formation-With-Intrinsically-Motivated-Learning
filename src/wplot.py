...
actuator_maps = np.load(
    f"{data_dir}/www/StoredGripActuatorMap.npy", allow_pickle=True
)
weight_data = np.load(
    f"{data_dir}/storage/000999/weights.npy", allow_pickle=True
)[0]
...
def plot_touch_map(actuator_maps, num_touch_sensors, grid_size, grid_side, ax):
    ...
    touch_map = touch_map.reshape(10, 4, grid_side, grid_side)
    touch_map = touch_map.transpose(2, 0, 3, 1).reshape(
    ...
    for x in np.linspace(0, 19, 6)[:-1]:
        ax.plot([x, x], [0, 49], c="white")
    for y in np.linspace(0, 49, 6)[1:]:
        ax.plot([0, 20], [y, y], c="white")
    ax.set_xlim(0, 19)
    ax.set_ylim(0, 49)
