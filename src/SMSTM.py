...
class SMSTM:
    def __init__(
        ...
        self.stm_layer = STM(
            ...
            radial_fun=radial2d,
            name=self.name,
            external_radial_prop=external_radial_prop,
        )
        ...
        self.dists = keras.layers.Input(
            shape=[self.out_num], name=f"dists_{self.name}"
        )
        self.inputs = keras.layers.Input(
            shape=[self.inp_num], name=f"input_{self.name}"
        )
    ...
    def updateParams(self, sigma=None, lr=None):
        ...
        keras.backend.set_value(
            self.model.get_layer(self.name).sigma,
            self.sigma,
        )
    ...
    def getRadials(self):
        return self.stm_layer.internal_radials

    def setRadials(self, radials):
        self.stm_layer.external_radials = radials
